...
if [ -z "${DOMAIN:-}" ]; then
  echo "[certbot-dns-auth] ERROR: DOMAIN is not set. Configure .deploy/certbot.env"
  exit 1
fi

if [ -z "${HOSTINGER_API_TOKEN:-}" ]; then
  echo "[certbot-dns-auth] ERROR: HOSTINGER_API_TOKEN is not set. Configure .deploy/certbot.env or .env.droplet"
  exit 1
fi
