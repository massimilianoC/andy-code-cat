...
test.describe("prompt tab", () => {
    ...
    test("shows the journalled generation for a turn that produced no code", async ({ page }) => {
        ...
        await page.goto(`${BASE_URL}/workspace/${projectId}?autoPrompt=${encodeURIComponent(QUESTION)}`);
        const response = await generation;
        expect(response.status(), "the generation itself must succeed").toBeLessThan(300);
        await response.finished();

        // The precondition, measured rather than assumed: this turn must really have no artifact,
        // or the test is exercising the path that already worked.
        await expect(page.getByText(/Nessun codice generato|No generated code/)).toBeVisible({ timeout: 30_000 });
