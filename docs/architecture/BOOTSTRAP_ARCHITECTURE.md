...
- When a snapshot is created from an assistant message (`sourceMessageId`), the backend patches the source conversation message with `metadata.snapshotId` and `metadata.mediaResolution`, so conversation-first surfaces can identify persisted artifacts without replaying client logic
- `platform_generated` assets persist first-class lineage inside `generationMetadata` (`conversationId`, `sourceMessageId`, `parentSnapshotId`, `mediaKey`, `semanticQuery`, `resolutionRoute`, `fallbackUsed`) for audit, analytics, and media-repair UX without introducing a second asset-lineage document
...
- MiniMax M3 parity implementation details, query paths, and correlation examples are tracked in `docs/reports/MINIMAX_M3_PARITY_IMPLEMENTATION_2026-06-02.md`
